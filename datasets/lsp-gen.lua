...
local function readJson(filename)
  ...
  for _, sample in ipairs(annot) do
    -- sample.img_paths = paths.concat('gen', 'mpii', 'images', sample.img_paths)
   ...
   if not paths.filep('gen/mpii-lsp/meanstd.t7') then
      ...
      for idx = 1, size_ do
         ...
         if dataset == 'LEEDS' then
            imgpath = paths.concat('gen/lsp', imgpath)
         else
            imgpath = paths.concat('gen/mpii/images', imgpath)
         end
...
function M.exec(opt, cacheFile)
   print('==> Create LSP dataset')
   local lspTrain, lspTest = readJson('data/lsp/LEEDS_annotations.json')

   -- Simplified for serialization
   trainData, trainLabels = Serialize(lspTrain)
   testData, testLabels = Serialize(lspTest)
   
   local train, test = {data=trainData, labels=trainLabels}, {data=testData, labels=testLabels}

   print(" | saving LSP dataset to " .. cacheFile)
   computeMean(train)
